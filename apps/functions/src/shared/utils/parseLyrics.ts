 ...
 * @returns 변환된 가사 배열
 */
export default function parseLyrics(lyricsString: string, duration: number): Lyric[] {
  // 1. 문자열을 줄바꿈(\n) 기준으로 나눕니다.
  ...
  const result: Lyric[] = parsedData.filter((item): item is Lyric => {
    return item.start !== undefined && item.end !== undefined && item.text !== undefined;
  });
