...
import getLyrics, { getLyricsOutputSchema } from "../tools/getLyrics";
...
export const addSongFlow = ai.defineFlow(
  ...
  async ({ title, artist }) => {
    ...
    // 가사 검색
    let lyrics: z.infer<typeof getLyricsOutputSchema>["lyrics"] | null = null;
    for (const [tryTitle, tryArtist] of combinations) {
      const { lyrics: tryLyrics } = await getLyrics({
        title: tryTitle,
        artist: tryArtist,
        duration,
      });
    ...
    // db 등록
    const doc = admin.firestore().collection("song").doc();
    await doc.set({
      title: koreanTitle ?? englishTitle ?? title,
      artist: koreanArtist ?? englishArtist ?? artist,
      album: null,
      duration,
